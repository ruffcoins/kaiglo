...
const InnerPageLayout = ({
    ...
    return (
        <main className="w-screen overflow-hidden max-w-[1500px] m-auto">
            ...
            <div className="lg:hidden fixed top-0 left-0 w-full z-10 flex items-center justify-between border-b-2 border-kaiglo_grey-disabled py-4 px-4 bg-white">
                ...
                {pathname.includes("/product") && (
                    <>
                        <ChevronLeftIcon className="w-6 h-6" onClick={() => router.back()} />
                        {pathname === "/category" && (
                            <h1 className="text-xl font-medium text-center flex-1">Category</h1>
                        )}
                        {pathname === "/product/featured" ? (
                            <h1 className="text-xl font-medium text-center flex-1">Featured Sales</h1>
                        ) : pathname === "/product/new-arrivals" ? (
                            <h1 className="text-xl font-medium text-center flex-1">New Arrivals</h1>
                        ) : pathname === "/product/group-buy" ? (
                            <h1 className="text-xl font-medium text-center flex-1">Group Buy</h1>
                        ) : pathname === "/product/top-selling" ? (
                            <h1 className="text-xl font-medium text-center flex-1">Top Selling</h1>
                        ) : (
                            pathname.includes("/product") && (
                                <h1 className="text-xl font-medium text-center flex-1">Product details</h1>
                            )
                        )}
            ...
            <div className="lg:hidden mt-16">{breadcrumbItems && <Breadcrumb items={breadcrumbItems} />}</div>
