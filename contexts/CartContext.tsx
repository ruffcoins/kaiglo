...
const CartProvider: React.FC<CartProviderProps> = ({ children }) => {
    ...
    const { removeItemFromCart, removeItemFromCartAsync } = useRemoveItemFromCart();
    ...
    useEffect(() => {
        ...
        if (user && Array.isArray(user.cart)) {
            cartToUse = user.cart.map((item) => convertCartItemToICacheCart(item, user.id));
        } else {
    ...
    const removeCheckedItems = async () => {
        const checkedItemsToRemove = cart.filter((item, index) => checkedItems[`${item.productId}-${index}`]);
        ...
        if (user) {
            ...
            const newCart = cart.filter((item, index) => !checkedItems[`${item.productId}-${index}`]);
        ...
        if (process.env.NODE_ENV === "production" && process.env.NEXT_PUBLIC_KAIGLO_ENV === "prod") {
    ...
    const getCheckedItems = useCallback(() => {
        return cart.filter((item, index) => checkedItems[`${item.productId}-${index}`]);
    }, [cart, checkedItems]);

    const getCheckoutTotal = useCallback(() => {
        return checkoutItems.reduce((total, item) => {
            return total + parseInt(item.quantity) * parseFloat(item.price.toString());
        }, 0);
    }, [checkoutItems]);
